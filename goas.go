...
func (g *Goas) parseModel(m *Model, modelName string, currentPackage string, knownModelNames map[string]bool) ([]*Model, error) {
	...
	if ok {
		...
		model := &Model{}
		typeInnerModels, err := g.parseModel(model, modelNameParts[0]+"."+modelNameParts[1], modelPackage, knownModelNames)
		if err != nil {
			return nil, err
		}
		fmt.Println(typeInnerModels)
		...
		m.Ref = referenceLink(modelPackage)
